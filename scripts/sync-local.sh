...
if [ ! -f "$DEST/mcp.json" ]; then
  ...
  echo "Seeded mcp.json (uses \${AGENT_READY_API_KEY}). Set that env var, or edit"
  echo "  $DEST/mcp.json"
  echo "with your Agent Ready Pro key so the MCP server can authenticate."
  echo "(Issue a key at https://agent-ready.dev/dashboard/api-keys.)"
fi
